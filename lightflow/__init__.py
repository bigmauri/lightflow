...
def get_proto_tasks(*l: list):
	...
	for _ in l:
		...
		for task in _:
			...
			_t_b, _t_c, _t_a = get_proto_commands(task["before_task"]), get_proto_commands(task["commands"]), get_proto_commands(task["after_task"])
			_t.update({"before_task": _t_b, "commands": _t_c, "after_task": _t_a})
